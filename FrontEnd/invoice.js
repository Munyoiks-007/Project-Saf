...
window.addEventListener("DOMContentLoaded", () => {
  ...
  // Auto-generate invoice number (e.g., INV-20250802-XYZ)
  ...
  downloadBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      doc.setFontSize(12);
      doc.text("Mojo Electrical Enterprise", 60, 20);
      doc.text("Reliability in Electrical Services", 60, 26);
      doc.text("P.O. Box 98664 - 80100, Mombasa", 60, 32);
      doc.text("Phone: +254 721 856 011 / 0731 120 072", 60, 38);
      doc.text("Email: gathucimoses@gmail.com", 60, 44);

      doc.setFontSize(14);
      doc.text("Sales Invoice", 10, 60);
      doc.setFontSize(12);
      doc.text(`Client: ${name}`, 10, 70);
      doc.text(`Date: ${date}`, 10, 78);
      doc.text(`Invoice No: ${number}`, 10, 86);
      ...
      document.querySelectorAll("#itemList tr").forEach((row, i) => {
        ...
        doc.text(`${i + 1}. ${item} | ${desc} | Qty: ${qty} | Price: ${price} | Total: ${total}`, 10, y);
        y += 10;
      });
